...
function Impact_17:bcRoleDamage(roleId,ret)
    ...
    if targetType == TargetType.Attacker then
        ...
        local target = recver.battle:GetRoleById(ret.senderId)
        if target == nil or not target.isValid or not target:IsAlive() then
            return
        end
        --队友伤害不触发
        if target.side == recver.side then
            return
        end
    ...
    local useRet = false
    if isUseImm then
        useRet = recver:CastSkill(skillExId,targetId)
    else
        useRet = recver:ActUseSkillByID(skillExId,targetId)
    end
