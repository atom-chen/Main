...
function BuffContainer:NotifyBuffChange()
    ...
    --战斗逻辑是顺序触发的，但是策划想要表现层面有并行关系
    --这个时候客户端逻辑事件的触发，顺序可能和逻辑不一致
    --buff状态不像伤害事件加减后可是等价的，互斥、更新、特效等经常和顺序相关
    --之前加过的seq表示顺序，错过的不再触发
    --但是表现上还是会经常遇到不一致的情况，并且很难排查
    --考虑到buff数量有上限，并且数据量本身比较小
    --因此NotifyBuffChange改成直接同步全数据，不再做增减操作，这个事件包含的就是当前版本的全数据

    local infos
    for _,buff in ipairs(self.buffs) do
        if  buff.isAlive and (buff.dirtyType == nil or buff.dirtyType ~= ImpactDirtyType.Remove)
                and buff:IsSync() then
...
function BuffContainer:GetBuffSync(isSyncAllAttrs)
    ...
    local infos = {}
    for _,buff in ipairs(self.buffs) do
        if buff:IsActiveValid() and (buff:IsSync() or isSyncAllAttrs) then
            local senderID = nil
            if buff.sender ~= nil then
                senderID = buff.sender.id
            end
...
function BuffContainer:IsStun()
    return self:AnyBuffIsSubClass(ImpactSubClass.Stun) 
        or self:AnyBuffIsSubClass(ImpactSubClass.StunBoss)
end
...
function BuffContainer:TmpRemoveAll()
    for _,buff in ipairs(buffs) do
        buff:FadeOut()
    end
    self:NotifyBuffChange()
...
function BuffContainer:TmpAddAll()
    for _,buff in ipairs(buffs) do
        buff:FadeIn()
    end
    self:NotifyBuffChange()
