...
function Scenario:OnRound(battle)
    self:CheckAndExcute()
end
...
function Scenario:CanFindActor(actor)
    if not self:CommonCheck() then
        return false
    end
...
function Scenario:IsActorAlive(actor)
    local role = self:FindActor(actor)
    if role == nil then return false end
    return role:IsAlive()
end

function Scenario:IsActorRound(actor)
    if not self:CommonCheck() then
        return false
    end
    local role = self:FindActor(actor)
    ...
    return battle.curRoundRole == role
end
...
function Scenario:IsRound(waveIndex,roundIndex)
    if not self:CommonCheck() then
        return false
    end
    local battle = self.battle
    if battle.state ~= BattleState.Round then
        return false
    end
    if battle.roundCount + 1 == roundIndex and battle.waveIndex == waveIndex then
        return true
...
function Scenario:Handle_GainSp(e)
    local battle = self.battle
    if battle == nil then
        return false
    end

    local hero = battle:GetHero(e.Side)
    if hero == nil then
        return false
    end
    hero:GainSP(e.SP)
