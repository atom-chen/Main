...
public class StoryModePrecond
{
    ...
    public void Reset()
    {
        ...
        StoryCardID = GlobeVar.INVALID_GUID;
    }
    ...
    public System.UInt64 StoryCardID;   // 从符灵故事进入故事模式时, 选择的卡牌GUID
}
...
public class StoryManager : MonoBehaviour
{
    ...
    public void SetPrecondition(ENTER_STORY_MODE_SOURCE enterMode, System.UInt64 cardGuid, int singleStoryId = GlobeVar.INVALID_ID, bool firstLoadingFinished = false)
    {
        Precondition.EnterMode = enterMode;
        Precondition.StoryCardID = cardGuid;
    ...
    {
        //Debug.LogErrorFormat("enter story line {0}, story {1}",
        //    GameManager.PlayerDataPool.m_nStoryLine,
        //    GameManager.PlayerDataPool.m_nStroyID);
        ...
        //Debug.LogErrorFormat("enter story mode: " + Precondition.EnterMode + ", card id " + Precondition.StoryCardID + ", force loading scene " + bForceLoadScene);

        StoryMode = true;

        InitStory(GameManager.PlayerDataPool.m_nStroyID, GameManager.PlayerDataPool.m_nStoryState, bCheckID, bForceLoadScene);
    ...
    private IEnumerator StartLeaveStoryMode(int nLastSceneID)
    {
        ...
        else
        {
            ...
            else if (TutorialManager.IsOpenTutorial && nStoryLineId == GlobeVar.TutorialCardLevelUp_StoryLineId &&
                    GameManager.PlayerDataPool.GetStoryFin(nStoryLineId) == 1 &&
                    false == TutorialManager.IsTutorialComplete(TutorialGroup.CardLevelUp, 1))
            {
                MainUI.m_TutorialGroupOnMenuBtn = TutorialGroup.CardLevelUp;
                MainUI.m_TutorialStepOnMenuBtn = 1;
                GameManager.ReqChangeScene(nLastSceneID);
            }
            else if (nStoryLineId == GlobeVar.TutorialQuestFinish_Start_StoryLineID && GameManager.PlayerDataPool.GetStoryFin(nStoryLineId) == 1 &&
                GameManager.PlayerDataPool.IsTutorialQuestAward(GlobeVar.TutorialQuestFinish_TutorialQuestId) &&
                false == TutorialManager.IsTutorialComplete(TutorialGroup.QuestFinish, 1))
            {
                // 新手剧情3第一次结束 且完成了对应新手任务 触发后续指引
                QuestBallWindow.m_TutorialGroupOnShow = TutorialGroup.QuestFinish;
                QuestBallWindow.m_TutorialStepOnShow = 1;

                GameManager.ReqChangeScene(nLastSceneID);
            }
            if (nStoryLineId == GlobeVar.TutorialQuartzEquip_StoryLineID && GameManager.PlayerDataPool.GetStoryFin(nStoryLineId) == 1 &&
                false == TutorialManager.IsTutorialComplete(TutorialGroup.QuartzEquip, 1))
            {
                //完成剧情后 进入星宿装备引导
                MainUI.m_TutorialGroupOnMenuBtn = TutorialGroup.QuartzEquip;
                MainUI.m_TutorialStepOnMenuBtn = 1;
                GameManager.ReqChangeScene(nLastSceneID);
            }
    ...
    void InitEnv(bool switchImm = false)
    {
        ...
        if (null == GameManager.EnvManager.CurEvnTable || m_CurStoryTable.Enviroment != GameManager.EnvManager.CurEvnTable.Id)
        {
            ...
            {
                bool imm = switchImm || m_CurStoryTable.EnviromentSwitchMode == 1;//|| !GameManager.storyManager.Precondition.FirstLoadingFinished;
                //Debug.LogErrorFormat("init story {0}, change scene, switch to {1}, imm {2}", GameManager.PlayerDataPool.m_nStroyID, m_CurStoryTable.Enviroment, imm);
    ...
    void InitStoryNPC()
    {
        ...
        {
            int nSceneNpcID = m_CurStoryTable.GetStroyNPCbyIndex(i);
            if (nSceneNpcID >= 0)
            {
                Tab_SceneNpc _tabSceneNpc = TableManager.GetSceneNpcByID(nSceneNpcID, 0);
                if (null == _tabSceneNpc)
                {
                    LogModule.ErrorLog("npc " + nSceneNpcID + " tab data not found");
                    break;
                }
                Obj_NPC npc = InitNPC(_tabSceneNpc, m_CurStoryTable.GetStroyNPCHeadFlagbyIndex(i));
                if (null != npc)
                {
                    npc.AddNpcMisc(_tabSceneNpc.MiscID);
                }
            }
        }
        ...
        if (m_CurStoryTable.NPCGroup != GlobeVar.INVALID_ID)
        {
            ...
            if (null != _tabGroup)
            {
                ...
                {
                    int nSceneNpcID = _tabGroup.GetNPCIDbyIndex(i);
                    if (nSceneNpcID >= 0)
                    {
                        Tab_SceneNpc _tabSceneNpc = TableManager.GetSceneNpcByID(nSceneNpcID, 0);
                        if (null == _tabSceneNpc)
                        ...
                        Obj_NPC npc = InitNPC(_tabSceneNpc, GlobeVar.INVALID_ID);
        ...
        foreach (int _key in m_LastStoryNPC.Keys)
        {
            if (m_CurNPC.ContainsKey(_key) == false)
            {
                ...
                {
                    //do something
                    //Debug.LogErrorFormat("destroy npc {0} in group", _key);
    ...
    //创建剧情NPC专用函数
    private Obj_NPC InitNPC(Tab_SceneNpc _tabSceneNpc, int npcHeadFlagIndex)
    {
        if (null == _tabSceneNpc)
        {
            return null;
        ...
        //判断是否重复
        if (m_LastStoryNPC.ContainsKey(_tabSceneNpc.Id))
        {
            //有了，直接插入
            Obj_NPC npc = m_LastStoryNPC[_tabSceneNpc.Id];
            npc.NpcHeadFlagIndex = npcHeadFlagIndex;

            //如果断线重连，修正位置
            if (!Precondition.FirstLoadingFinished)
            {
                Vector3 resetPos = new Vector3(_tabSceneNpc.PosX, 0.0f, _tabSceneNpc.PosZ);
                GetStoryNPCTrans(_tabSceneNpc.Id, m_CurStoryTable, ref resetPos);
                ObjManager.ForceSetObjPos(npc, resetPos.x, resetPos.z);
            }

            m_CurNPC.Add(_tabSceneNpc.Id, npc);

            return npc;
        }

            // 默认scene npc表中位置
            Vector3 defaultPos = new Vector3(_tabSceneNpc.PosX, 0.0f, _tabSceneNpc.PosZ);
            Vector3 defaultRot = new Vector3(0.0f, _tabSceneNpc.FaceDirection, 0.0f);
            ...
            if (!Precondition.FirstLoadingFinished)
            {
                GetStoryNPCTrans(_tabSceneNpc.Id, m_CurStoryTable, ref defaultPos);
            }

            defaultPos.y = _tabSceneNpc.PosY;

            //创建NPC
            Obj_Init_Data _cardInfo = new Obj_Init_Data();
            _cardInfo.m_CreatePos = defaultPos;
            _cardInfo.m_CreateRot = defaultRot;
            _cardInfo.m_nModelID = Utils.GetRealCharModel(_tabSceneNpc.DataID);
            _cardInfo.m_bShowNameBoard = true;
            _cardInfo.m_nDyeColorId = Utils.GetRealDyeColorId(_tabSceneNpc.DyeColorID);
            Obj_NPC _missionNPC = ObjManager.CreateNPC(_cardInfo, Utils.GetRealName(_tabSceneNpc.DataID));
            if (null != _missionNPC)
            {
                //将NPC插入到当前节点中
                m_CurNPC.Add(_tabSceneNpc.Id, _missionNPC);

                //设置SceneNPC的ID
                _missionNPC.SceneNPCID = _tabSceneNpc.Id;
                _missionNPC.DialogID = _tabSceneNpc.DialogID;

                //根据表格配置的不同状态确定NPC是否可以被选中和是否跟随选中者
                if ((int)NPC_SECELT_TYPE.UNFACETO == _tabSceneNpc.CanBeSelect)
                {
                    _missionNPC.CanBeSelect = true;
                    //_missionNPC.FaceToSelecter = false;
                }
                else if ((int)NPC_SECELT_TYPE.UNSELECTED == _tabSceneNpc.CanBeSelect)
                {
                ...
                _missionNPC.FocusMaxDistance = _tabSceneNpc.HelloRange;
                _missionNPC.FaceToSelecter = (_tabSceneNpc.HelloFaceTo != -1 && _tabSceneNpc.HelloRange > 0);

                _missionNPC.NpcHeadFlagIndex = npcHeadFlagIndex;

                //挂特效
                if (_tabSceneNpc.EffectID != GlobeVar.INVALID_ID)
                {
                    _missionNPC.PlayEffect(_tabSceneNpc.EffectID);
                }
            }
            return _missionNPC;
    }
    ...
    public void OnStoryFinish()
    {
        ...
        if (null != m_CurStoryTable)
        {
            ...
            else
            {
                ...
                {
                    //如果BranchList只有一个，则无分支，直接进入剧情
                    if (GameManager.m_bOffLineMode)
                    {
                        GameManager.PlayerDataPool.m_nStroyID = storyBranchList[0];

                        //if (StoryFadeInOutController.Instance() != null && !StoryFadeInOutController.Instance().IsAutoClose())
                        //{
                        //    Tab_Story tabStory = TableManager.GetStoryByID(storyBranchList[0], 0);

                        //    if (null != tabStory && tabStory.SceneID != (int)GameManager.RunningScene)
                        //    {
                        //        UIManager.CloseUI(UIInfo.FadeInOut);
                        //    }
                        //}

                        InitStory(storyBranchList[0], (int)STORY_STATE.NORMAL, true, false);
                    }
                    else
                    {
    ...
    //事件离开触发
    public void OnEventLeave(bool refreshNpc)
    {
        if (refreshNpc)
        {
            foreach (var npc in m_CurNPC.Values)
    ...
    public int GetCurTutorialSceneNpcId()
    {
        ...
        //    return GlobeVar.TutorialStory_SceneNpcId;
        //}
        //else if (m_CurStoryTable.LindID == GlobeVar.TutorialBattle_StoryLineId &&
        //    m_CurStoryTable.Id == GlobeVar.TutorialBattle_StoryId)
        //{
        //    return GlobeVar.TutorialBattle_SceneNpcId;
        //}
        //else if (m_CurStoryTable.LindID == GlobeVar.TutorialStory3_StoryLineId &&
        //    m_CurStoryTable.Id == GlobeVar.TutorialStory3_StoryId)
        //{
        //    return GlobeVar.TutorialStory3_SceneNpcId;
        //}
        //else if (m_CurStoryTable.LindID == GlobeVar.TutorialBattle3_StoryLineId &&
        //    m_CurStoryTable.Id == GlobeVar.TutorialBattle3_StoryId)
        //{
        //    return GlobeVar.TutorialBattle3_SceneNpcId;
        //}
        //else
        //{
    ...
    bool CanMove()
    {
        ...
        {
            if (m_CurStory.EventList[i].State == STORY_EVENT_STATE.ACITVE)
            {
                hit = true;
    ...
    static public bool StoryLineAvailable(int storylineId, System.UInt64 cardGuid)
    {
        return StoryLineAvailableWithReason(storylineId, cardGuid) == StoryLineNAReason.AVAILABLE;
    }

    public enum StoryLineNAReason { AVAILABLE, UNKNOWN_ERROR, LEVEL, STORY_FLAG, HAS_CARD, CARD_AWAKE, CARD_LEVEL, CARD_INTIMACY, CARD_STAR, CARD_BATTLE_CNT }
    static public StoryLineNAReason StoryLineAvailableWithReason(int storylineId, System.UInt64 cardGuid)
    {
        ...
        //卡牌标记
        if (_tabLine.CollectCardID != GlobeVar.INVALID_ID)
        {
            if (false == GameManager.PlayerDataPool.CollectionData.IsGetCard(_tabLine.CollectCardID))
            {
                return StoryLineNAReason.HAS_CARD;
            }
        }

        //if (_tabLine.AwakeCardID != GlobeVar.INVALID_ID)
        //{
        //    if (false == GameManager.PlayerDataPool.IsCardAwake(_tabLine.CollectCardID))
        //    {
        ...
        //}
        Card card = GameManager.PlayerDataPool.PlayerCardBag.GetCard(cardGuid);
        if (_tabLine.CardLevelID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.CardLevelID)
                return StoryLineNAReason.HAS_CARD;
            if (card.Level < _tabLine.CardLevel)
                return StoryLineNAReason.CARD_LEVEL;
        }
        if (_tabLine.CardIntimacyID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.CardIntimacyID)
                return StoryLineNAReason.HAS_CARD;
            if (card.IntimacyLevel < _tabLine.CardIntimacy)
                return StoryLineNAReason.CARD_INTIMACY;
        }
        if (_tabLine.CardStarID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.CardStarID)
                return StoryLineNAReason.HAS_CARD;
            if (card.Star < _tabLine.CardStar)
                return StoryLineNAReason.CARD_STAR;
        }
        if (_tabLine.AwakeCardID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.AwakeCardID)
                return StoryLineNAReason.HAS_CARD;
            if (card.Awakening <= 0)
                return StoryLineNAReason.CARD_AWAKE;
        }
        if (_tabLine.CardBattleCnt > GlobeVar.INVALID_ID)
        {
            if (card == null)
                return StoryLineNAReason.HAS_CARD;
            if (card.BattleCnt < _tabLine.CardBattleCnt)
                return StoryLineNAReason.CARD_BATTLE_CNT;
        }
    ...
    static public HashSet<StoryLineNAReason> StoryLineAvailableWithAllReason(int storylineId, System.UInt64 cardGuid)
    {
        ...
        if (_tabLine.CardLevelID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.CardLevelID)
                ret.Add(StoryLineNAReason.HAS_CARD);
            if (card != null && card.Level < _tabLine.CardLevel)
        ...
        if (_tabLine.CardIntimacyID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.CardIntimacyID)
                ret.Add(StoryLineNAReason.HAS_CARD);
            if (card != null && card.IntimacyLevel < _tabLine.CardIntimacy)
        ...
        if (_tabLine.CardStarID > GlobeVar.INVALID_ID)
        {
            if (card == null || card.CardId != _tabLine.CardStarID)
                ret.Add(StoryLineNAReason.HAS_CARD);
            if (card != null && card.Star < _tabLine.CardStar)
        ...
        if (_tabLine.CardBattleCnt > GlobeVar.INVALID_ID)
        {
            if (card == null)
                ret.Add(StoryLineNAReason.HAS_CARD);
            if (card != null && card.BattleCnt < _tabLine.CardBattleCnt)
                ret.Add(StoryLineNAReason.CARD_BATTLE_CNT);
        }
    ...
    {
        if (!StoryManager.StoryLineAvailable(storylineId, cardGuid))
            return false;
    ...
    static public bool HasNewCardBiography(Card card)
    {
        if (card == null)
            return false;
        ...
        for (int i = 0; i < tabCardStory.getBiographyIntimacyCount(); ++i)
        {
            ...
            if (card.IntimacyLevel >= inLv && !PlayerPreferenceData.IsClickedCardBiography(card.CardId, i))
                return true;
        }
