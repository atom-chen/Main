...
public class QuartzEquipWindow : MonoBehaviour
{
    ...
    private int m_ChooseIndex = GlobeVar.INVALID_ID;     //当前选中的Card ID
    private List<int> m_OldSetIdList = new List<int>();

    private const int OrbmentAttrClassCount = 8;

    private int m_CurChooseSlotIndex = GlobeVar.INVALID_ID;    //当前选中的槽
    public int CurChooseSlot { get { return m_CurChooseSlotIndex == GlobeVar.INVALID_ID ? GlobeVar.INVALID_ID : m_CurChooseSlotIndex + 1; } }
    ...
    void OnEnable()
    {
        ...
        if (m_Card == null || m_CardList == null)
        {
            return;
        }
    ...
    void OnDisable()
    {
        ...
        m_CurChooseSlotIndex = GlobeVar.INVALID_ID;
    }
    ...
    void InitOrbmentSetAttr()
    {
        ...
        for (int i = 0; i < activeSetIdList.Count && i < m_SetAttr.Length; i++)
        {
            ...
            if (tSet != null)
            {
                m_SetAttr[i].text = Utils.GetFormatSetAttr(tSet);      //往套装属性里写入格式化字符串
            }
        }
    ...
    public void HandleOrbmentSlotItemClick(OrbmentSlotItem slotItem)
    {
        ...
        for (int i = 0; i < m_QuartzSlotItem.Length; i++)
        {
            if (m_QuartzSlotItem[i] == slotItem)
            {
                if (OrbmentController.Instance != null)
                {
                    if (m_CurChooseSlotIndex == i)
                    {
                        OrbmentController.Instance.HandleOrbmentSlotItemClick_Same(slotItem, i);
                    }
                    //点击了不同的槽位
                    else
                    {
                        m_CurChooseSlotIndex = i;
                        OrbmentController.Instance.HandleOrbmentSlotItemClick_Diff(slotItem, i);
                    }
