...
namespace NetFrame
{
    ...
    public class UserToken
    {
        ...
        public Socket conn;            //用户连接

        public SocketAsyncEventArgs receiveSAEA;              //用户异步接收网络数据对象
        public SocketAsyncEventArgs sendSAEA;       //用户异步发送网络数据对象

        public LengthEncode LE;                //消息长度编码器
        public LengthDecode LD;                //消息长度解码器
        public encode encode;                  //消息编码
        public decode decode;                  //消息解码
        ...
        public AbsHandlerCenter center;//消息处理中心

        List<byte> cache = new List<byte>();//接收消息队列

        private bool isReading = false;//标志位
        private bool isWriting = false;
        Queue<byte[]> writeQueue = new Queue<byte[]>();//写入消息队列
