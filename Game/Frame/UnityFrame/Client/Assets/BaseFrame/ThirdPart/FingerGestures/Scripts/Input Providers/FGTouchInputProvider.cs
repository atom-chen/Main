...
public class FGTouchInputProvider : FGInputProvider 
{
    ...
    void UpdateFingerTouchMap()
    {
        ...
        for( int i = 0; i < Input.touchCount; ++i )
        {
            ...
            if( 0 <= fingerIndex && fingerIndex < finger2touchMap.Length && fingerIndex < maxTouches)
    ...
    {
        if (fingerIndex >= 0 && fingerIndex < finger2touchMap.Length && fingerIndex < maxTouches)
        {
            int touchIndex = finger2touchMap[fingerIndex];
            if (touchIndex >= 0 && touchIndex < Input.touchCount)
            {
