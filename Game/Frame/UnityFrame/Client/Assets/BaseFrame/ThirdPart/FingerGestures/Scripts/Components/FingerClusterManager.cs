...
public class FingerClusterManager : MonoBehaviour 
{
	...
	public void Update()
    {
		...
		for(int fingerIndex=0, count = fingersRemoved.Count; fingerIndex<count; fingerIndex++)
        //foreach( FingerGestures.Finger finger in fingersRemoved )
        {
            ...
            for( int i = clusters.Count - 1; i >= 0; --i )
            {
                ...
                if( cluster.Fingers.Remove( fingersRemoved[fingerIndex] ) )
                {
		...
		for(int fingerIndex=0, count = fingersAdded.Count; fingerIndex<count; fingerIndex++)
        {
            // try to add finger to existing cluster
			Cluster cluster = FindExistingCluster( fingersAdded[fingerIndex] );

            // no valid active cluster found for that finger, create a new cluster
            if( cluster == null )
            {
                cluster = NewCluster();
				cluster.StartTime = fingersAdded[fingerIndex].StarTime;             
            }

            // add finger to selected cluster
			cluster.Fingers.Add( fingersAdded[fingerIndex] );
        }
    }
