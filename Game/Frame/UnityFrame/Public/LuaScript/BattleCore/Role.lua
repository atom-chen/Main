...
function Role:AI_ShouldUseIter(skillAiId,counter)
    ...
    local tab = tabMgr.GetByID("SkillAI",skillAiId)    
    if tab == nil then
        return true
    end

    --各个条件检查
    local pass = self:AI_ShouldUse_HPLe(tab)
        and self:AI_ShouldUse_AnyHPLe(tab)
        and self:AI_ShouldUse_OurAverageHPLe(tab)
        and self:AI_ShouldUse_IsEnv(tab)
        and self:AI_ShouldUse_OurAllBetterEnv(tab)
        and self:AI_ShouldUse_OurAllBuffCount(tab)
        and self:AI_ShouldUse_OurRoleDead(tab)
    ...
    if tab.Or ~= -1 then
        return self:AI_ShouldUseIter(tab.Or,counter+1)
    else
        return pass   
    end
end
...
--自己血量小于等于
function Role:AI_ShouldUse_HPLe(tab)
    if tab.HPLe ~= -1 then
        if self:GetHPPercent10000() > tab.HPLe then
            return false
        end
    end
    return true
...
--全员平均血量，小于等于
function Role:AI_ShouldUse_OurAverageHPLe(tab)
    if tab.OurAverageHPLe ~= -1 then
        ...
        if percent > tab.OurAverageHPLe then
            return false
        end
    end
    return true
...
--任意一员，血量小于等于
function Role:AI_ShouldUse_AnyHPLe(tab)
    if tab.AnyHPLe ~= -1 then
        local roles = self.battle:GetRoleAlies(self)            
        local pass = false
        for _,role in ipairs(roles) do
            if role:IsAlive() then
                if role:GetHPPercent10000() <= tab.AnyHPLe then
                    pass = true
                    break
                end
            end
...
--是否处于某个环境下
function Role:AI_ShouldUse_IsEnv(tab)
    if tab.IsEnv == -1 then
        return true
    end

    return self.battle.curEnvType == tab.IsEnv
end

--是否处于我方优势环境下，根据我方优势符灵数量，对比对方优势符灵数量
function Role:AI_ShouldUse_OurAllBetterEnv(tab)
    if tab.OurAllBetterEnv == - 1 then
        return true
    end
...
--我方特定buff数量和是否大于指定数量
function Role:AI_ShouldUse_OurAllBuffCount(tab)
...
--我方角色死亡数量大于等于
function Role:AI_ShouldUse_OurRoleDead(tab)

    if tab.OurRoleDeadCount <= 0 then
        return true
    end

    if self.battle:CountRoleDead(self.side) < tab.OurRoleDeadCount then
        return false
    end
...
function Role:CastSkillByIndex(index,targetId,data)
    ...
    if ret then
        self:SendMessage("msgAfterUseSkillByIndex",index)
    end
    return ret
end
...
function Role:CastSkill(skillExId,targetId,data)
    ...
    if not SkillProcess.Utils.IsRelive(tabEx) then
        --非复活技能，目标需要存活
        if not self:IsValidSkillTarget(targetId) then
...
function Role:IsValidSkillTarget(targetId)
...
function Role:CheckCanUseSkill(skillid,islog)
    ...
    if tabEx.BaseID == 1162 then
        local targets = SkillProcess.GetMarkedTargets(self,1162210)
