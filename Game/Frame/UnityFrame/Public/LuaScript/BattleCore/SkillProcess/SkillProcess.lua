...
function SkillProcess:Init(skillId,caster,battle)
    ...
    if self.tabEx ~= nil and (self.tabEx.__script_type == nil and self.tabEx.LogicID == 4) then
    ...
    if self:IsReliveSkill() then
        ...
        for _,hitID in ipairs(tabEx.Hit) do
            ...
            if isValidHit then
                ...
                --不能行动了(打断)
                if not self.caster:CanAct() then
                    break
                end
            end
        end
...
function SkillProcess:SendToRandCond(targets,count,hitTab,hitResults,func)
    ...
    --忽略掉不合法的角色
    common.removec(targets,function(r) return SkillProcess.TargetFilter(r) or (not func(r)) end)
    targets = self.battle:RandomSelect(targets,count)    
...
function SkillProcess:SendToMarked(hitTab,hitResults)
    ...
    local targets = SkillProcess.GetMarkedTargets(self.caster,buffId)
    if targets == nil then
        return
    end
    for _,target in ipairs(targets) do
...
function SkillProcess:SplashTo(hitTab,hitResults)
    ...
    local filter_last = function(r) return r == lastTarget or SkillProcess.TargetFilter(r) end
    ...
    while count < maxCount do
        ...
        --删掉不合法的
        common.removec(targets,filter_last)
        if #targets == 0 then
            break
        end
        --随机选取不是之前目标的角色
...
function SkillProcess.GetMarkedTargets(role,impactId)
    local targets = role.battle:GetRoleEnemies(role)
    if targets == nil then
        return nil
    end
    common.removec(targets,function(r) 
        return SkillProcess.TargetFilter(r) or not r:HasImpact(impactId,role)
    end)
    return targets
end
...
function SkillProcess:SendImpacts2Target(targetId,hitTab,isDirective)
    ...
    for i,impactId in ipairs(hitTab.Impact) do
        ...
        if isRefix then
            --计算效果命中影响
            chance = SkillProcess.CalcImpactChance(chance
        ...
        if isValidImpact and chance > 0 and self:CanSendImpact(impactId,hitTab,chance) then
    ...
    if hitId ~= -1 then
        ...
        for i,impactId in ipairs(hitTab.Impact) do
            ...
            if isRefix then
                ...
                chance = SkillProcess.CalcImpactChance(chance
                            ,self.caster:GetAttrValue(AttrType.ImpactChance)
                            ,reliveRole:GetAttrValue(AttrType.ImpactResist)
                        )
            end

            if impactId > 0 and chance > 0 and self:CanSendImpact(impactId,hitTab,chance) then
...
--impact是否可以上
function SkillProcess.CalcImpactChance(chance,enhance,resist)
    local chanceRefix = math.ceil(10000 + enhance - resist)
    chanceRefix = math.max(chanceRefix,0)
    chance = math.ceil(chance * (chanceRefix / 10000))
    --min 1000,max 9000
    chance = common.clamp(chance,1000,9000)
    return chance
end

function SkillProcess:CanSendImpact(impactId,hitTab,chance)

    if self.battle.gm_ImpactChanceFull then
        return true
    end
