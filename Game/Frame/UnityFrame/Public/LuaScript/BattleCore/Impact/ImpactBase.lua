...
function Impact:SendMessage(methodName,...)
    ...
    --叠加子buff，暂时不处理消息
    -- if self.layerChilds ~= nil then
    --     for _,childImpact in ipairs(self.layerChilds) do
    --         if childImpact.isAlive then
    --             childImpact:SendMessage(methodName,...)
    --         end
    --     end
    -- end
end
...
function Impact:ImpactEffected()
    common.debuglog("impact effected",self.impactId)
    if self.skillInfo ~= nil then
