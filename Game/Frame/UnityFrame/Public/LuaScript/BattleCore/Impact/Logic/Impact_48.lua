--特殊伤害，根据指定的属性，计算攻击力
--新攻击值=属性类型1*修正值+属性类型2*修正值+属性类型3*修正值
--属性类型1是攻击，修正是10000,其他-1,则和0号逻辑等价

--参数：
--1，技能系数
--2，技能固定值系数
--3，同一技能相同impact多次命中衰减系数
...
local Impact_48 = class("Impact_48",Impact)

function Impact_48:OnActive()
    Impact_48.__base.OnActive(self)
    self.damping = self.tab.Param[3]
    self:DoCalc()
end

function Impact_48:DoCalc()
    local tab = self.tab
    ...
    local attr1 = tab.Param[4]
    local attr1Val = tab.Param[5]
    local attr2 = tab.Param[6]
    local attr2Val = tab.Param[7]
    local attr3 = tab.Param[8]
    local attr3Val = tab.Param[9]
    ...
    local a1 = sender:GetAttrValue(attr1)
    attack = math.ceil(a1 * (attr1Val / 10000))
    if attr2 ~= -1 then
        local a2 = sender:GetAttrValue(attr2)
        attack = attack +  math.ceil(a2 * (attr2Val / 10000))
    end
    if attr3 ~= -1 then
        local a3 = sender:GetAttrValue(attr3)
        attack = attack +  math.ceil(a3 * (attr3Val / 10000))
    end


    local baseAttack = math.ceil( (attack * p) / (recver:GetAttrValue(AttrType.Defense)  + p) )
...
function Impact_48:OnImpact()
    Impact_48.__base.OnImpact(self)
