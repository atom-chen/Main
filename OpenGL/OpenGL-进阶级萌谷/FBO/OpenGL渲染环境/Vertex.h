...
public:
	...
	~FrameBuffer();
public:
	void AttachColorBuffer(const char* bufferName,GLenum attachment,const unsigned& width, const unsigned& height);//Attch颜色缓冲区
	void AttachDepthBuffer(const char* bufferName, const unsigned& width, const unsigned& height);//Attch深度缓冲区
public:
	void Finish();//设置结束
	void Begin();
