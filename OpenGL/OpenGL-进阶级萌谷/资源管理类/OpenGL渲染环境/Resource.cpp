...
GLuint ResourceManager::GetPic(const char* bmpPath)
{
	...
	{
		//去读文件
		GLuint texture=CreateTexture2DFromPNG(bmpPath);
		printf("加载图片%s\r\n", bmpPath);
		m_mTexture.insert(mType(name, texture));
		return texture;
	}
}
...
GLuint ResourceManager::GetProgram(const char* vertexShaderPath, const char* fragmentShaderPath)
{
	...
	else
	{
		...
		m_mProgram.insert(mType(name, program));
		return program;
	}
}
...
//----------------------------------------------------------------
///---------------------------Public End--------------------------
std::map<string, GLuint> ResourceManager::m_mTexture;//纹理map<文件名，纹理>
//GPU程序管理
std::map<string, GLuint> ResourceManager::m_mProgram;//Program map<vert+frag,程序ID>
//ObjModel管理
...
GLuint ResourceManager::CreateTexture2D(unsigned char* pixelData, int width, int height, GLenum type)
{
