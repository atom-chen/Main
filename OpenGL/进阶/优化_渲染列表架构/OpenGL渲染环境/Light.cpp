...
void Light::SetDirection(float x, float y, float z)//设置方向光的照射方向（位置在无穷远处）
{
	this->m_Direction = vec4(x, y, z, 0);
}
void Light::SetDirection(const vec3& direction)
{
	this->m_Direction = vec4(direction, m_Direction.w);
}
void Light::SetPosition(float x, float y, float z)//设置位置
{
	this->m_Position = vec4(x, y, z, m_Position.w);
}
void Light::SetPosition(const vec3& position)
{
	this->m_Position = vec4(position, m_Position.w);
}
...
Light::Light() :m_Direction(0,1,1,0)
{
...
void Light::SetType(LIGHT_TYPE type)
{
	...
	{
	case LIGHT_INVALID:
		m_Position = vec4(0, 0, 0, 0);
		m_Color = LightColor();
		break;
	case LIGHT_DIRECTION:
		m_Position = vec4(m_Position.x, m_Position.y, m_Position.z, 0);
		break;
	case LIGHT_POINT:
		m_Position = vec4(m_Position.x, m_Position.y, m_Position.z, 1);
		break;
	case LIGHT_SPECULAR:
		m_Position = vec4(m_Position.x, m_Position.y, m_Position.z, 1);
		break;
