...
public:
    ...
    virtual void    onInit()
    {
        ...
        _shader.initialize();
        _textureId  =   loadTexture("res/data/image/woodfloor.tga");
        _texGround  =   loadTexture("res/data/image/1.jpg");
        ...
        for (float x = 0 ;x < 100 ; x += 10 )
        {
            ...
            {
                CELLNode    node;
                node.setPosition(float3(x,0,z));
                node.setRenderable(&_model);
                node.update(true);
                _arNodes.push_back(node);
            }
        }
