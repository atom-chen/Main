...
void Shader::SetTextureCube(const char* name, GLuint texture)
{
...
void Shader::Bind(const float *M, const float* V, const float *P, const float *IT )
{
	...
	if (m_mUniformTextureCube.size() > 0)
	{
		for (auto it = m_mUniformTextureCube.begin(); it != m_mUniformTextureCube.end(); it++)
		{
